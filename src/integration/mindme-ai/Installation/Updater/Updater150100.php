 ...
 * widget table with the MindMeAiBundle class (required for the widget
 * parameters to be persisted and for the Home editor to offer them). Fresh
 * installations get them from the plugin config (config.yml), already-installed
 * platforms get them here.
 *
 * Data migration: environments installed before this migration still hold the
 * landing widgets under the HomeBundle class and store their parameters in the
 * `claro_home_landing_widget` table. This updater switches the registered
 * widget class to MindMeAiBundle and moves the parameter rows into
 * `claro_mindme_landing_widget` (the legacy table is backed up first).
 */
class Updater150100 extends Updater
{
    ...
    /**
     * The legacy table created by the HomeBundle migration.
     */
    private const LEGACY_TABLE = 'claro_home_landing_widget';

    /**
     * The backup table created before migrating the legacy parameters.
     */
    private const BACKUP_TABLE = 'claro_home_landing_widget_bak';
     ...
     * and switches already-registered instances (HomeBundle) to MindMeAiBundle.
     */
    private function ensureLandingWidgets(): void
    {
        $mindmePlugin = $this->om
            ->getRepository(Plugin::class)
            ->findOneBy(['vendorName' => 'Claroline', 'bundleName' => 'MindMeAiBundle']);
        ...
        foreach (self::LANDING_WIDGETS as $name) {
            ...
            if ($widget) {
                // legacy registration (HomeBundle) or a class mismatch: switch
                // the widget to the MindMeAiBundle implementation so the
                // serializer and the parameter table lookup match.
                if (LandingWidget::class !== $widget->getClass()) {
                    $widget->setClass(LandingWidget::class);

                    if ($mindmePlugin) {
                        $widget->setPlugin($mindmePlugin);
                    }

                    $this->om->persist($widget);

                    $this->logger?->info(sprintf('Landing widget "%s" class switched to MindMeAiBundle.', $name));
                }
    ...
    /**
     * Moves the legacy parameter rows from `claro_home_landing_widget` into
     * `claro_mindme_landing_widget` (idempotent).
     *
     * The legacy table is backed up before any row is touched. Rows already
     * present in the new table (by widget instance) are left untouched.
     */
    private function migrateLegacyParameters(): void
    {
        $connection = $this->om->getConnection();

        if (!$this->tableExists($connection, self::LEGACY_TABLE)) {
            return;
        }

        $legacyRows = $connection->fetchAllAssociative(sprintf(
            'SELECT * FROM %s',
            self::LEGACY_TABLE
        ));

        if (0 === count($legacyRows)) {
            return;
        }

        $this->backupLegacyTable($connection);

        foreach ($legacyRows as $row) {
            $instanceId = $row['widgetInstance_id'] ?? null;

            if (empty($instanceId)) {
                continue;
            }

            $alreadyMigrated = $connection->fetchOne(
                'SELECT id FROM claro_mindme_landing_widget WHERE widgetInstance_id = ?',
                [$instanceId]
            );
            ...
            $connection->insert('claro_mindme_landing_widget', [
                // the DBAL driver already returns the JSON column as a string
                // (a valid JSON document), so it must be stored as-is
                'parameters' => $row['parameters'],
                'widgetInstance_id' => $instanceId,
            ]);

            $this->logger?->info(sprintf('Landing widget parameters migrated for widget instance #%s.', $instanceId));
        }

        // delete the migrated rows from the legacy table
        $connection->executeStatement(sprintf('DELETE FROM %s', self::LEGACY_TABLE));

        $this->logger?->info(sprintf('Migrated %d legacy landing widget parameter row(s).', count($legacyRows)));
    }

    private function backupLegacyTable($connection): void
    {
        if (!$this->tableExists($connection, self::BACKUP_TABLE)) {
            ...
            $this->logger?->info(sprintf('Legacy table backed up as "%s".', self::BACKUP_TABLE));

            return;
        }

        // the backup table already exists (e.g. a previous run or an
        // environment where the migration already created it): refresh it
        // with DML only so the operation stays transaction-safe
        $connection->executeStatement(sprintf('DELETE FROM %s', self::BACKUP_TABLE));
        $connection->executeStatement(sprintf(
        ...
        $this->logger?->info(sprintf('Legacy table backup refreshed in "%s".', self::BACKUP_TABLE));
    }
