...
final class Version20240613082245 extends AbstractMigration
{
    ...
    {
        $this->addSql('
            INSERT INTO claro_ordered_tool (uuid, tool_name, context_name, context_id, entity_order, is_public)
            SELECT UUID() AS uuid, "progression" AS tool_name, "workspace" AS context_name, w.uuid AS context_id, 0 AS entity_order, 0 AS is_public
            FROM claro_workspace AS w  
            WHERE NOT EXISTS (
                SELECT o2.id 
                FROM claro_ordered_tool AS o2 
                WHERE o2.tool_name = "progression"
