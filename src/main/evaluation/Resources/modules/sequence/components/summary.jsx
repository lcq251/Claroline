...
const PathSummary = (props) => {
        ...
        <>
          ...
          <span className="step-status">
            <span className={classes('fa fa-fw fa-flag-checkered', {
              'not_started': Object.values(props.stepsProgression).length <= 0 || Object.values(props.stepsProgression).map( (step) => ['seen'].includes(step) ).length !== props.sequence.steps.length
            } )} />
          </span>
        </>
      ),
  ...
  const summaryLinks = baseLinks.concat(
    props.sequence.steps.map(getStepSummary)
  ).concat(endLink)*/

  if (0 !== props.sequence.steps.length) {
    return (
      <ContentSummary
        className={props.className}
        links={props.sequence.steps.map(getStepSummary)}
        noCollapse={true}
      />
    )
  }
