...
namespace Claroline\VideoPlayerBundle\Installation\Updater;
...
class Updater150000 extends Updater
{
    ...
    public function postUpdate(): void
    {
        $videoType = $this->om->getRepository(ResourceType::class)->findOneBy([
            'name' => VideoResource::getName(),
        ]);

        $updateResources = $this->connection->prepare(
            'UPDATE claro_resource_node SET resource_type_id = :resourceType WHERE mime_type LIKE "video%"'
        );
